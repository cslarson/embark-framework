...
class Pipeline {
  ...
  build(abi, contractsJSON, path, callback) {
    ...
    this.writeContractsJS(contractsJSON);
  ...
  writeContractsJS(contractsJSON) {
    for (let contractName in contractsJSON) {
      let filePath = utils.joinPath(fs.dappPath(), this.buildDir, 'contracts', `${contractName}.js`);
      let code = this.buildContractJS(contractName);
